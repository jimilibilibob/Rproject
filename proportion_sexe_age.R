...
gg_global<-  ggplot(data) +
  ...
  geom_text(data = subset(data,sexe=="femme"), stat = "count", aes(label = round(..count../row_data*100,2), y = (..count..)/row_data*-100))+
  
  
  geom_bar(data = subset(data,sexe=="homme"),aes(y = (..count..)/row_data*100)) +
  geom_text(data = subset(data,sexe=="homme"),stat = "count", aes(label = round(..count../row_data*100,2), y = (..count..)/row_data*100))+
