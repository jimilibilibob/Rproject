...
#NEW_N1 : client ayant adhere au cours de l'ann?e N-1 (date debut adhesion)
...
plot_ly(data.frame(cbind(slices,levels)), labels = ~levels,values=~slices, type="pie")%>%
  layout(title = "Répartition des clients",
...
client_CA <- entetes[,list(TOTALCA = sum(TIC_TOTALTTC)),
                     by=list(IDCLIENT,year(TIC_DATE))]
